...
do {
    ...
    selectedIndex = console.readNumber(`Escoge opción? [1-${OPTIONS.length}]: `) - 1;
    let outputText = '';
    switch (OPTIONS[selectedIndex]) {
        case 'Ver Países':
            for (let i = 0; i < NAMES.length; i++) {
                let suffix = ', ';
                const remainder = NAMES.length - i - 1;
                if (remainder === 1) {
                    suffix = ' y ';
                } else if (remainder === 0) {
                    suffix = '.';
                }
                outputText += NAMES[i] + suffix;
            }
            break;
        case 'Buscar país':
            ...
            outputText = `El país "${responseCountry}" ${isIncluded ? 'sí' : 'no'} está incluido`;
            break;
    }
    if (outputText.length > 0) {
        outputText += '\n';
    }
    console.writeln(outputText);
} while (OPTIONS[selectedIndex] !== 'Salir');
