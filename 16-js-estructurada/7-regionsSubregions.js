...
const REGION_NAME = 0;
const SUBREGIONS_ARRAY = 1;
const result = [];
for (let i = 0; i < REGIONS.length; i++) {
    let regionIndex;
    for (let j = 0; regionIndex === undefined && j < result.length; j++) {
        if (result[j][REGION_NAME] === REGIONS[i]) {
            regionIndex = j;
        }
    }
    if (regionIndex === undefined) {
        ...
        result[regionIndex][SUBREGIONS_ARRAY] = [];
    }

    let subregionIndex;
    for (let j = 0; subregionIndex === undefined && j < result[regionIndex][SUBREGIONS_ARRAY].length; j++) {
        if (result[regionIndex][SUBREGIONS_ARRAY][j] === SUBREGIONS[i]) {
            subregionIndex = j;
        }
    }
    if (subregionIndex === undefined && SUBREGIONS[i] !== undefined) {
        subregionIndex = result[regionIndex][SUBREGIONS_ARRAY].length;
        result[regionIndex][SUBREGIONS_ARRAY][subregionIndex] = SUBREGIONS[i];
    }
}

for (let i = 0; i < result.length; i++) {
    console.writeln(`${result[i][REGION_NAME]}:`);
    for (let j = 0; j < result[i][SUBREGIONS_ARRAY].length; j++) {
        console.writeln(`\t${result[i][SUBREGIONS_ARRAY][j]}`);
    }
}
